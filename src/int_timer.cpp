...
struct tmrTemplate intervalTimer(int numSets, int numReps, float setRest, bool hiFirst, float hiLength, float lowLength, float repRest, float prepare, float cool) {
  struct tmrTemplate tmp {
    numSets, 
    numReps, 
    (unsigned long) (1000. * setRest),
    hiFirst,
    (unsigned long) (1000. * hiLength),
    (unsigned long) (1000. * lowLength),
    (unsigned long) (1000. * repRest),
    (unsigned long) (1000. * prepare),
      ...
      if (phaseElapsed < intTimer->hiLength) { //in exercise
        ...
        if (intTimer->hiFirst) { // we must switch to low then
          *intPhase = TMRLOW;
          ...
          if (intProgress->rep == intTimer->reps) { //this was the last rep in set 
            if (intProgress->set == intTimer->sets) { //this was the last rep of the last set
              *intPhase = TMRCOOL;
              intProgress->rep = 0;
              intProgress->set++;
